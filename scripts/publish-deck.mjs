...
function usage() {
  console.log(`Usage:
  npm run deck:publish -- --dir .deck-build/<slug> --slug <slug> [--version <version>] [--upload]

Default mode is a dry run. Add --upload only after reviewing the object plan.
The source directory must contain deck.json plus every relative asset it references.
...
function parseArguments(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (argument === "--upload") {
      values.upload = true;
    } else if (["--dir", "--slug", "--version"].includes(argument)) {
...
function uploadObject(source, key, cacheControl) {
...
if (!options.dir || !options.slug) {
  ...
  console.log(`  decks/${options.slug}/latest.json`);

  if (!options.upload) {
    console.log("Dry run only. Re-run with --upload after reviewing this plan.");
  } else {
    for (const object of objects) {
      uploadObject(object.file, object.key, "public, max-age=31536000, immutable");
    }
    ...
    console.log(`Published: ${PUBLIC_ORIGIN}/decks/${options.slug}/latest.json`);
  }
}
